///////////   This file is styled by command "indent -kr shfRplc.cpp"
///////////   @author Zhang Liqian, Li Wanpeng
//////////////////////////////////////////////////////////////
#include "llvm/IR/LLVMContext.h"
///#include "llvm/ADT/ArrayRef.h"    // NEW
...
class pattern_rec {
  private:
    ...
    void pattern_diff(const SmallVector < int, 16 > &P, int m, int k,
		      SmallVector < int, 16 > &vec) {
	for (int i = 0; i < m - k; i++)
	    vec.push_back(P[i + k] - P[i]);
    } void number_count(const SmallVector < int, 16 > &vec, int beg,
			int end, const SmallVector < int, 16 > &value,
  ...
  public:
    pattern_rec(const SmallVector < int, 16 > &vec, unsigned ttl, bool b1,
		bool b2) {
	sin_mask = vec;
	signature.clear();
	...
	/* @version 1.3 
	 * Note that it's a sequential case-check.
	 * So if one mask can belong to two type,
	 * the prior one will be recognized
	 *
	 * End of version 1.3
	 */
	...
	NCadd(0) NCadd(1) NCadd(2) NCadd(opLen) NCadd(1 -
						      len) NCadd(-opLen)
	...
	if (__DEBUG)
	    VECTOR_DUMP(signature);
	_DEBUG << "\n";
	/* end of version 1.4 */
	/* @version 1.1        
	   // check rotate pattern
	   if ((candidate[1] == len-2)&&(candidate[4] == 1))
	   return Rotate;
	   *  end of version 1.1        
	 */
	/* @version 1.2
	 * replace rotate check
	 */
	// check rotate pattern ( one lane)
	if (((candidate[1] == len - 2)
	     && (candidate[4] == 1 /*1-opLen?? */ )
	     && (len == opLen))) {
	...
	/* @version 1.3 */
	// check shift right logical pattern
	if (candidate[1] + candidate[0] >= len - 2) {
	    bool succ = 1;
	    int i = 1;
	    for (; i < (len - candidate[0] - 1); i++)
		succ &= (signature[i] == 1);
	    succ &= (signature[i] == opLen - len + 1);
	    for (i = (len - candidate[0]); i < len; i++)
		succ &= (signature[i] == 0);
	    if (succ && isV2Zero) {	// Pay attention! now the progress still needs V2 be zeroinitializer
		eigenvalue = signature[0];
		return ShiftRightLogical;
	    }
	}
  ...
  private:
	...
	BinaryOperator *boInst1 =
	    BinaryOperator::Create(BinaryOperator::Xor,
				   (Value *) boInstShl,
				   (Value *) boInstLShr, "Xor",
				   (Instruction *) svi);
	// bitcast 
	BitCastInst *bcInstRot =
	    new BitCastInst((Value *) boInst1, (Type *) resType,
			    "bitcastRot",
			    (Instruction *) svi);
	...
	if (laneNum == 1) {	// No lane handle
	    ...
	    bcInst1 =
		new BitCastInst(op1,
				Type::getIntNTy(svi->getContext(), bitLen),
				"bitcast1", (Instruction *) svi);
		...
		tcInst1 =
		    new TruncInst(bcInst1, intType, "trunc1",
				  (Instruction *) svi);
	    }
	    // Step 2:
	    ...
	    Constant *offset =
		ConstantInt::getSigned(intType, offsetValue);
	    BinaryOperator *boInst1;
	    /* @version 1.2
	    ...
	    if (!isRotate) {
		boInst1 =
		    BinaryOperator::Create(BinaryOperator::Shl,
					   (resLen <
					    bitLen) ? (Value *) tcInst1
					   : (Value *)
					   bcInst1, (Value *) offset,
					   "shl1", (Instruction *) svi);
		BitCastInst *bcInst2L =
		    new BitCastInst((Value *) boInst1, (Type *) resType,
				    "bitcast2L", (Instruction *) svi);
		svi->replaceAllUsesWith((Value *) bcInst2L);
	    } else
		boInstShl =
		    BinaryOperator::Create(BinaryOperator::Shl,
	    ...
	    BitCastInst *bcInst1 =
		new BitCastInst(op1, vecType, "bcInst1",
				(Instruction *) svi);
	    // Step 2:
		...
		ConstantInt::getSigned(intType, offsetValue);
	    Constant *offsetVec =
		ConstantVector::getSplat(laneNum, offset);
	    /* @version 1.5 */
	    if (isRotate) {
		boInstShl =
		    BinaryOperator::Create(BinaryOperator::Shl,
					   (Value *) bcInst1,
					   (Value *) offsetVec, "shl1",
					   (Instruction *) svi);
		return true;
	    } else {
		BinaryOperator *boInst1 =
		...
		if (resLen < bitLen) {
		    ...
		    BitCastInst *bcInst3 =
			new BitCastInst((Value *) boInst1,
					Type::getIntNTy(svi->getContext(),
							bitLen), "bcInst3",
					(Instruction *) svi);
		    tcInst1 =
			new TruncInst((Value *) bcInst3,
				      Type::getIntNTy(svi->getContext(),
						      resLen), "trunc1",
				      (Instruction *) svi);
		}
		...
		BitCastInst *bcInst2L =
		    new BitCastInst((resLen < bitLen) ? (Value *) tcInst1
				    : (Value *) boInst1, (Type *) resType,
				    "bitcast2L", (Instruction *) svi);
		// Step 4:
		// delete the ShuffleVector result uses
		svi->replaceAllUsesWith((Value *) bcInst2L);
...
/* @version 1.2 */
	...
	if (laneNum == 1) {	// No lane handle
	    ...
	    if (!isRotate) {
		boInst1 =
		    BinaryOperator::Create(BinaryOperator::LShr,
					   (Value *) bcInst1,
					   (Value *) offset, "LShr",
					   (Instruction *) svi);
		BitCastInst *bcInst2R =
		    new BitCastInst((Value *) boInst1, (Type *) resType,
				    "bitcast2R", (Instruction *) svi);
		svi->replaceAllUsesWith((Value *) bcInst2R);
	    } else
		boInstLShr =
		    BinaryOperator::Create(BinaryOperator::LShr,
	...
	// try to replace 
	CastInst *testInst1 =
	    CastInst::CreateZExtOrBitCast((Value *) bcInst1,
					  Type::getIntNTy(svi->
							  getContext(),
							  resLen), "res",
					  (Instruction *) svi);
	...
	BitCastInst *bcInst2 =
	    new BitCastInst((Value *) testInst1, (Type *) resType,
			    "bitcast2",
			    (Instruction *) svi);
	//
	//_DEBUG<<"???\n";
    ...
    /* @version 1.1
     * Unfortunately, we can't resolve replaceWithMerge at this Pass level...
     * Backend need interfering
     */
    ...
    /* @version 1.3
     * add instruction generation for replaceWithBlend
     * after version 1.2, we consider input which is well-formed.
     */
    bool replaceWithBlend() {
	// isFull determines whether to truncate
	Value *op1 = svi->getOperand(0);
	Value *op2 = svi->getOperand(1);
	VectorType *v1Type = cast < VectorType > (op1->getType());
	unsigned bitLen = v1Type->getBitWidth();
	unsigned opLen = v1Type->getNumElements();
	VectorType *resType = cast < VectorType > (svi->getType());
	unsigned resLen = resType->getBitWidth();
	bool isFull = (bitLen == resLen);
	/* Step 1:
	 *     construct the cond selective vector
	 */
	Type *tI1 = Type::getInt1Ty(svi->getContext());
	SmallVector < Constant *, 16 > cond;
	for (unsigned i = 0; i < sin_mask.size(); i++) {
	    APInt num(1, (sin_mask[i] == i) ? 1 : 0);
	    cond.push_back(Constant::getIntegerValue(tI1, num));
	}
	for (unsigned i = sin_mask.size(); i < opLen; i++) {
	    APInt num(1, 1);
	    cond.push_back(Constant::getIntegerValue(tI1, num));
	}
	Constant *cond_vec = ConstantVector::get(cond);
	/* Step 2:
	 *    construct select inst
	 */
	SelectInst *slInst1 =
	    SelectInst::Create((Value *) cond_vec, op1, op2, "slInst1",
			       (Instruction *) svi);

	// Step 2.5&3:
	// delete the ShuffleVector result uses
	// If truncation is needed, add another three transformation
	if (isFull) {
	    svi->replaceAllUsesWith((Value *) slInst1);
	} else {		// truncate the opLen-length selected result to mask_len-length result
	    BitCastInst *bcInst1 = new BitCastInst((Value *) slInst1,
						   Type::getIntNTy(svi->
								   getContext
								   (),
								   bitLen),
						   "bcInst1",
						   (Instruction *) svi);
	    TruncInst *tcInst1 = new TruncInst((Value *) bcInst1,
					       Type::getIntNTy(svi->
							       getContext
							       (), resLen),
					       "tcInst1",
					       (Instruction *) svi);
	    BitCastInst *bcInst2 =
		new BitCastInst((Value *) tcInst1, (Type *) resType,
				"bcInst2",
				(Instruction *) svi);
	    svi->replaceAllUsesWith((Value *) bcInst2);
	}
  ...
  public:
    instruction_rep(ShuffleVectorInst * sv, const SmallVector < int,
		    16 > &vec, unsigned ln, PatternName pn,
		    unsigned chrct) {
	svi = sv;
	sin_mask = vec;
	laneNum = ln;
...
namespace {
	...
	unsigned lane_detect(const SmallVector < int, 16 > &vec,
			     unsigned len, unsigned constantMask) {
	    // constantMask is zero if their does not exist "the second constant vector"
	    bool succ = true;
	    if (len < 1) {
		errs() <<
		    "error condition in lane_detect, maybe mask: 01234567\n";
		return 0;
	    }
	...
	virtual bool runOnBasicBlock(llvm::BasicBlock & bb) {
	    _DEBUG << "Begin shufflevector pattern recognition\n";
	    ...
	    for (llvm::BasicBlock::iterator bbiter = bb.begin();
		 bbiter != bb.end(); bbiter++) {
		Instruction *inst = bbiter;
		ShuffleVectorInst *svi =
		    dyn_cast < ShuffleVectorInst > (inst);
		// shfvct detected
		if (svi != nullptr) {
		    ...
		    _DEBUG << (operChanged ? "v1 and v2 changed" :
			       "v1 and v2 not changed") << '\n';
		    _DEBUG << "mask now: ";
		    if (__DEBUG)
			VECTOR_DUMP(mask) _DEBUG << '\n';
		    //
			...
			for (unsigned i = 0; i < total_len; i++)	// update mask
			    mask[i] =
				mask[i] / opLen * len2 + mask[i] % opLen;
		    ...
		    pattern_rec recognizer(sin_mask, opLen, isV2Splat,
					   isV2Zero);
		    ...
		    instruction_rep replacer(svi, sin_mask, 1 << laneNo,
					     typeId, eigenvalue);
		    if (replacer.replace()) {
			_DEBUG << "replacement succeeds. \n";
			toBeDeleted.push_back(svi);
			isModified = true;
		    } else
			_DEBUG <<
			    "Error occurs during instruction replacement!!!\n";
		    _DEBUG << "five steps finish\n";
		}
	    }			//  end of instruction's for loop
	    if (isModified) {
		_DEBUG <<
		    "Extra Step: remove original ShuffleVector Instructions\n";
...
X("shfRplc",
  "ShuffleVectorInst pattern recognization and replacement Pass", false,
  false);
