...
bool replaceWithZeroExtend() {
	 ...
	 */
	SelectInst * slInst1 = SelectInst::Create((Value *)cond_vec, op1, op2, "slInst1", (Instruction *)svi);
	...
	if (isFull) {
	    ...
	    BitCastInst* bcInst1 = new BitCastInst((Value*)slInst1, Type::getIntNTy(svi->getContext(), bitLen), "bcInst1", (Instruction*)svi);
	    TruncInst* tcInst1 = new TruncInst((Value*)bcInst1, Type::getIntNTy(svi->getContext(), resLen), "tcInst1", (Instruction*)svi);
	    BitCastInst* bcInst2 = new BitCastInst((Value*)tcInst1, (Type*)resType, "bcInst2", (Instruction*)svi);
	    svi->replaceAllUsesWith((Value*)bcInst2);
	}
	return true;
    }
...
namespace {
	...
	unsigned lane_detect(const SmallVector<int, 16>& vec, unsigned len, unsigned constantMask) {
	    ...
	    if (i+len < vec.size())   // protect in case of halfMaskLen*2 > total_len
	        succ &= ((vec[i]<len)&&(vec[i+len]-vec[i]==len) || (constantMask!=0)&&(vec[i+len]==constantMask)&&(vec[i]==constantMask));
	...
	static char ID;
		...
		ShuffleVectorInst *svi = dyn_cast<ShuffleVectorInst>(inst);
			...
			_DEBUG << "Error occurs during instruction replacement!!!\n";
